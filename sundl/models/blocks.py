...
class CrossModalAttention(tf.keras.layers.Layer):
  ...
  def call(self, features):
    ...
    input_mode_attention_weights /= tf.sqrt(tf.cast(self.attention_units),input_mode_q.dtype)
...
class CrossModalSpatialAttention(tf.keras.layers.Layer):
  ...
  def call(self, features):
    ...
    attention_weights = tf.nn.softmax(tf.matmul(q, k, transpose_b=True) / tf.sqrt(tf.cast(self.attention_units),q.dtype), axis=-1)
