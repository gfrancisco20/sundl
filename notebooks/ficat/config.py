...
def rollAvg(timeserie, window_h):
  """ Function to go from the 1h serie to longer avgs """
  return timeserie[['sw_v']].rolling(f'{window_h}H', closed = 'right').mean()
PATH_SW_TS_12H =  PATH_TIME_SERIES/'sw_omni_12h.csv'
PATH_SW_TS_6H =  PATH_TIME_SERIES/'sw_omni_6h.csv'
PATH_SW_TS_1H = PATH_TIME_SERIES/'sw_omni_1h.csv'

PATH_CMES = PATH_ROOT_DRIVE_DS/'Events_Catalogs/CMEs_Chierichini_v1.csv' 
 
 
 
F_PATH_CHUNKS_SW = lambda chunk_width, buffer : PATH_FOLDS/f'sw_chunks_{chunk_width.days}x{buffer.days}.csv'
F_PATH_FOLDS_SW = lambda chunk_width, buffer, testDate, vAvg_h : PATH_FOLDS/f'sw_folds_{testDate.strftime("%Y%m%d")}_{chunk_width.days}x{buffer.days}_{vAvg_h}h.pkl'
F_PATH_TEST_SW  = lambda bufferTest, testDate,vAvg_h : PATH_FOLDS/f'sw_test_{testDate.strftime("%Y%m%d")}_{bufferTest.days}_{vAvg_h}h.csv'
